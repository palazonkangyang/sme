...
use SME\Http\Requests\Supplier\ProductCollection\StoreRequest;
use SME\Http\Requests\Supplier\ProductCollection\UpdateRequest;

class ProductCollectionController extends SupplierController
{
    ...
    public function index(Request $request)
    {
        ...
        $user_name = \Auth::supplier()->user()->id;
        $collections = ProductValue::where('supplier_id' ,'=',$user_name)->join("product_collection_locations as pt", "location_id", "=", "pt.id")->select('product_values.*','pt.name')->get()->all();
    ...
    public function postVerification($id)
    {
        ...
        return redirect()->route("supplier.collection.product.add", array("ticket_id" =>$this->data["collection"]->ticket_id,"product_id" => $this->data["collection"]->product_id));
    }
    ...
    private function validateTicket($ticket_no,$product_no)
    {
        ...
        if (!$ticket || empty($ticket_no)) {
            return redirect()->route("supplier.collection.product.verification.post")->with("alert", [
                "success" => false,
        ...
        if ($product_no == "" || !\Auth::supplier()->user()->products()->where("product_id", $product_no)->first()) {
            return redirect()->route("supplier.collection.product.verification.post")->with("alert", [
                "success" => false,
    ...
    public function create($ticket_no,$product_no)
    {
        ...
        $this->data["locations"] = \Auth::supplier()->user()->allLocations()->orderBy("name")
    ...
    public function store(StoreRequest $request)
    {
        ...
        return redirect()->route("supplier.collection.product.verification")->with("alert", [
            "success" => TRUE,
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        return redirect()->route("supplier.collection.product.edit", [$ticket->id])->with("alert", [
            "success" => TRUE,
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route("supplier.collection.all")->with("alert", [
            "success" => TRUE,
