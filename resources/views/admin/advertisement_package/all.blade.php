                ...
                @if($items->count() > 0)
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            ...
                            <tr>
                                <td>#</td>
                                <th>{!! ListView::sortColumn("Advertisement Package Name", "advertisement_package") !!}</th>
                                <th>{!! ListView::sortColumn("Advertisement Price", "advertisement_price") !!}</th>
                                <th>{!! ListView::sortColumn("Features", "options") !!}</th>
                            ...
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td>
                                        {{ $item->advertisement_package }} <br/>
                                        <a href="{!! route("admin.advertisement_package.edit", [$item->id]) !!}"><i
                                                    class="fa fa-edit"></i> Edit</a>

                                        
                                            &nbsp;&nbsp;<a class="delete-confirm"
                                                           href="{!! route("admin.advertisement_package.delete", [$item->id]) . "?_token=" . csrf_token() !!}"><i
                                                        class="fa fa-trash"></i> Delete</a>
                                       
                                    </td>
                                    
                                    <td>{{ $item->advertisement_price }}</td>
                                    <td>{{ $item->options }}</td>
