            ...
            <div class="box">
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                <th>{!! ListView::sortColumn("First Name", "first_name") !!}</th>
                                <th>{!! ListView::sortColumn("Last Name", "last_name") !!}</th>
                                <th>{!! ListView::sortColumn("Role", "role") !!}</th>
                            ...
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td>
                                        {{ $item->email }} <br/>
                                        <a href="{!! route("admin.account.profile", [$item->id]) !!}"><i
                                                    class="fa fa-edit"></i> View</a>&nbsp;&nbsp;
                                        <a href="{!! route("admin.account.profile.edit", [$item->id]) !!}"><i
                                                    class="fa fa-edit"></i> Edit</a>
                                        @if($item->role != 0)
                                            &nbsp;&nbsp;<a class="delete-confirm"
                                                           href="{!! route("admin.account.delete", [$item->id]) . "?_token=" . csrf_token() !!}"><i
                ...
                @else
                    <div class="alert alert-info">
                        @if( Input::get("q") )
                            Hmmm .., There is nothing matched for the keyword <i><strong>{{ Input::get("q") }}</strong></i>
                            . You may try to search with other keyword.
                        @else
                            Hmmm .., There is nothing to display here yet.
                        @endif
                    </div>
                @endif
            </div>
